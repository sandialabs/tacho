...
template <typename value_type> int driver_chol(const int m, const bool verbose) {
  int max_iter = 1;
  ...
  int r_val = 0;
  {
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
      ...
      int lwork(0);
#if defined(KOKKOS_ENABLE_CUDA)
      Tacho::Lapack<value_type>::potrf_buffersize(handle_lapack, CUBLAS_FILL_MODE_UPPER, m, A.data(), m, &lwork);
      ...
      timer.reset();
      {
        Tacho::Trsv<Tacho::Uplo::Upper, Tacho::Trans::ConjTranspose, Tacho::Algo::OnDevice>::invoke(
            handle_blas, Tacho::Diag::NonUnit(), A, x);
        Kokkos::fence();
        Tacho::Trsv<Tacho::Uplo::Upper, Tacho::Trans::NoTranspose, Tacho::Algo::OnDevice>::invoke(
            handle_blas, Tacho::Diag::NonUnit(), A, x);
        Kokkos::fence();
      }
      {
