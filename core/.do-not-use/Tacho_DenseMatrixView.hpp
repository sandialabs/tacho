...
template <typename ValueType, typename DeviceType> struct DenseMatrixView {
public:
...
public:
  ...
  DenseMatrixView() : _offm(0), _offn(0), _m(0), _n(0), _rs(0), _cs(0), _buf(NULL), _future() {}

  KOKKOS_INLINE_FUNCTION
  DenseMatrixView(value_type *buf, const ordinal_type m, const ordinal_type n)
      : _offm(0), _offn(0), _m(m), _n(n), _rs(1), _cs(m), _buf(buf), _future() {}

  KOKKOS_INLINE_FUNCTION
  DenseMatrixView(const DenseMatrixView &b)
      : _offm(b._offm), _offn(b._offn), _m(b._m), _n(b._n), _rs(b._rs), _cs(b._cs), _buf(b._buf), _future() {}

  KOKKOS_INLINE_FUNCTION
  value_type &operator[](const ordinal_type k) const { return _buf[k]; }

  KOKKOS_INLINE_FUNCTION
  value_type &operator()(const ordinal_type i, const ordinal_type j) const {
    return _buf[(i + _offm) * _rs + (j + _offn) * _cs];
  }

  KOKKOS_INLINE_FUNCTION
  void set_view(const DenseMatrixView &base, const ordinal_type offm, const ordinal_type m, const ordinal_type offn,
  ...
  KOKKOS_INLINE_FUNCTION
  void set_view(const ordinal_type offm, const ordinal_type m, const ordinal_type offn, const ordinal_type n) {
    _offm = offm;
    _m = m;
    _offn = offn;
    _n = n;
  ...
  KOKKOS_INLINE_FUNCTION
  void set_view(const ordinal_type m, const ordinal_type n) {
    _offm = 0;
    _m = m;
    _offn = 0;
    _n = n;
  ...
  KOKKOS_INLINE_FUNCTION
  void attach_buffer(const ordinal_type rs, const ordinal_type cs, const value_type *buf) {
    _rs = rs;
    _cs = cs;
    _buf = const_cast<value_type *>(buf);
  }
...
KOKKOS_INLINE_FUNCTION void clearFutureOfBlocks(const MatrixOfBlocksViewType &H) {
...
template <typename MemberType, typename MatrixOfBlocksViewType>
KOKKOS_INLINE_FUNCTION void clearFutureOfBlocks(/* */ MemberType &member, const MatrixOfBlocksViewType &H) {
  const ordinal_type m = H.extent(0);
  const ordinal_type n = H.extent(1);
  Kokkos::parallel_for(Kokkos::TeamThreadRange(member, n), [&](const int &j) {
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(member, m), [&](const int &i) { H(i, j).set_future(); });
  });
}

template <typename MatrixOfBlocksViewType>
KOKKOS_INLINE_FUNCTION void setMatrixOfBlocks(const MatrixOfBlocksViewType &H, const ordinal_type m,
                                              const ordinal_type n, const ordinal_type mb, const ordinal_type nb) {
  const ordinal_type bm = H.extent(0);
  const ordinal_type bn = H.extent(1);

  for (ordinal_type j = 0; j < bn; ++j) {
    const ordinal_type jbeg = j * nb, jtmp = jbeg + nb, jend = jtmp > n ? n : jtmp,
                       jdiff = (jend > jbeg) * (jend - jbeg);

    for (ordinal_type i = 0; i < bm; ++i) {
      const ordinal_type ibeg = i * mb, itmp = ibeg + mb, iend = itmp > m ? m : itmp,
                         idiff = (iend > ibeg) * (iend - ibeg);

      H(i, j).set_view(ibeg, idiff, jbeg, jdiff);
    }
  }
}
...
template <typename MatrixOfBlocksViewType>
KOKKOS_INLINE_FUNCTION void setMatrixOfBlocks(const MatrixOfBlocksViewType &H, const ordinal_type m,
                                              const ordinal_type n, const ordinal_type mb) {
  setMatrixOfBlocks(H, m, n, mb, mb);
}

template <typename MemberType, typename MatrixOfBlocksViewType>
KOKKOS_INLINE_FUNCTION void setMatrixOfBlocks(/* */ MemberType &member, const MatrixOfBlocksViewType &H,
                                              const ordinal_type m, const ordinal_type n, const ordinal_type mb,
...
template <typename MatrixOfBlocksViewType, typename BaseBufferPtrType>
KOKKOS_INLINE_FUNCTION void attachBaseBuffer(const MatrixOfBlocksViewType &H, const BaseBufferPtrType ptr,
                                             const ordinal_type rs, const ordinal_type cs) {
...
template <typename MemberType, typename MatrixOfBlocksViewType, typename BaseBufferPtrType>
KOKKOS_INLINE_FUNCTION void attachBaseBuffer(/* */ MemberType &member, const MatrixOfBlocksViewType &H,
                                             const BaseBufferPtrType ptr, const ordinal_type rs,
                                             const ordinal_type cs) {
  const ordinal_type m = H.extent(0);
  const ordinal_type n = H.extent(1);
  Kokkos::parallel_for(Kokkos::TeamThreadRange(member, n), [&](const int &j) {
    Kokkos::parallel_for(Kokkos::ThreadVectorRange(member, m),
                         [&](const int &i) { H(i, j).attach_buffer(rs, cs, ptr); });
  });
}

template <typename MatrixOfBlocksViewType, typename MemoryPoolType>
KOKKOS_INLINE_FUNCTION void allocateStorageByBlocks(const MatrixOfBlocksViewType &H, const MemoryPoolType &pool) {
...
template <typename MatrixOfBlocksViewType, typename MemoryPoolType>
KOKKOS_INLINE_FUNCTION void deallocateStorageByBlocks(const MatrixOfBlocksViewType &H, const MemoryPoolType &pool) {
  typedef typename MatrixOfBlocksViewType::value_type dense_block_type;
  typedef typename dense_block_type::value_type value_type;
...
copyElementwise(const DenseMatrixView<ValueType, ExecSpace> &F,
  ...
  const ordinal_type hm = H.extent(0), hn = H.extent(0), fm = F.extent(0), fn = F.extent(0);
...
template <typename ValueType, typename ExecSpace>
KOKKOS_INLINE_FUNCTION void copyElementwise(const DenseMatrixView<DenseMatrixView<ValueType, ExecSpace>, ExecSpace> &H,
...
/// A = P B
template <typename DenseMatrixViewType, typename OrdinalTypeArray>
inline void applyRowPermutationToDenseMatrix(const DenseMatrixViewType &A, const DenseMatrixViewType &B,
