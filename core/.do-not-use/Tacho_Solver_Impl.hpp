...
Solver<VT, ST>::Solver()
    : _transpose(0), _mode(0), _order_connected_graph_separately(0), _m(0), _nnz(0), _ap(), _h_ap(), _aj(), _h_aj(),
      _perm(), _h_perm(), _peri(), _h_peri(), _m_graph(0), _nnz_graph(0), _h_ap_graph(), _h_aj_graph(), _h_perm_graph(),
      _h_peri_graph(), _N(nullptr), _L0(nullptr), _L1(nullptr), _L2(nullptr), _verbose(0), _small_problem_thres(1024),
      _serial_thres_size(-1), _mb(-1), _nb(-1), _front_update_mode(-1), _levelset(0), _device_level_cut(0),
      _device_factor_thres(64), _device_solve_thres(128), _variant(2), _nstreams(16), _max_num_superblocks(-1) {}
...
/// common options
///
template <typename VT, typename ST> void Solver<VT, ST>::setVerbose(const ordinal_type verbose) { _verbose = verbose; }

template <typename VT, typename ST>
void Solver<VT, ST>::setSmallProblemThresholdsize(const ordinal_type small_problem_thres) {
...
template <typename VT, typename ST>
void Solver<VT, ST>::setMatrixType(const int symmetric, // 0 - unsymmetric, 1 - structure sym, 2 - symmetric
...
template <typename VT, typename ST>
void Solver<VT, ST>::setOrderConnectedGraphSeparately(const ordinal_type order_connected_graph_separately) {
...
/// tasking options
///
template <typename VT, typename ST> void Solver<VT, ST>::setSerialThresholdsize(const ordinal_type serial_thres_size) {
  _serial_thres_size = serial_thres_size;
}

template <typename VT, typename ST> void Solver<VT, ST>::setBlocksize(const ordinal_type mb) { _mb = mb; }

template <typename VT, typename ST> void Solver<VT, ST>::setPanelsize(const ordinal_type nb) { _nb = nb; }

template <typename VT, typename ST> void Solver<VT, ST>::setFrontUpdateMode(const ordinal_type front_update_mode) {
  _front_update_mode = front_update_mode;
}

template <typename VT, typename ST>
void Solver<VT, ST>::setMaxNumberOfSuperblocks(const ordinal_type max_num_superblocks) {
...
///
template <typename VT, typename ST> void Solver<VT, ST>::setLevelSetScheduling(const bool levelset) {
  _levelset = levelset;
}

template <typename VT, typename ST>
void Solver<VT, ST>::setLevelSetOptionDeviceLevelCut(const ordinal_type device_level_cut) {
  _device_level_cut = device_level_cut;
}

template <typename VT, typename ST>
void Solver<VT, ST>::setLevelSetOptionDeviceFunctionThreshold(const ordinal_type device_factor_thres,
                                                              const ordinal_type device_solve_thres) {
  _device_factor_thres = device_factor_thres;
  _device_solve_thres = device_solve_thres;
}

template <typename VT, typename ST> void Solver<VT, ST>::setLevelSetOptionAlgorithmVariant(const ordinal_type variant) {
...
template <typename VT, typename ST> void Solver<VT, ST>::setLevelSetOptionNumStreams(const ordinal_type nstreams) {
  _nstreams = nstreams;
}
...
/// get interface
///
template <typename VT, typename ST> ordinal_type Solver<VT, ST>::getNumSupernodes() const { return _nsupernodes; }

template <typename VT, typename ST> typename Solver<VT, ST>::ordinal_type_array Solver<VT, ST>::getSupernodes() const {
  return _supernodes;
}

template <typename VT, typename ST>
typename Solver<VT, ST>::ordinal_type_array Solver<VT, ST>::getPermutationVector() const {
  return _perm;
}

template <typename VT, typename ST>
typename Solver<VT, ST>::ordinal_type_array Solver<VT, ST>::getInversePermutationVector() const {
  return _peri;
}

// internal only
template <typename VT, typename ST> int Solver<VT, ST>::analyze() {
  int r_val(0);
...
template <typename VT, typename ST> int Solver<VT, ST>::analyze_linear_system() {
  if (_verbose) {
...
template <typename VT, typename ST> int Solver<VT, ST>::analyze_condensed_graph() {
  if (_verbose) {
...
template <typename VT, typename ST> int Solver<VT, ST>::initialize() {
  if (_verbose) {
  ...
  if (_m < _small_problem_thres) {
    ...
    new (_N) numeric_tools_type(_m, _ap, _aj, _perm, _peri, _nsupernodes, _supernodes, _gid_super_panel_ptr,
...
template <typename VT, typename ST> int Solver<VT, ST>::factorize(const value_type_array &ax) {
  switch (_mode) {
  case Cholesky:
    factorize_chol(ax);
    break;
...
template <typename VT, typename ST> int Solver<VT, ST>::factorize_chol(const value_type_array &ax) {
  if (_verbose) {
  ...
  if (_m < _small_problem_thres) {
    ...
    timer.reset();
    _A = value_type_matrix_host("A", _m, _m);
    auto h_ax = Kokkos::create_mirror_view(host_memory_space(), ax);
    ...
    if (_levelset) {
      if (_variant == 0)
        _L0->factorizeCholesky(ax, _verbose);
      else if (_variant == 1)
        _L1->factorizeCholesky(ax, _verbose);
      else if (_variant == 2)
        _L2->factorizeCholesky(ax, _verbose);
    }
#if !defined(KOKKOS_ENABLE_CUDA)
    else if (nthreads == 1) {
      if (_nb < 0)
      ...
      if (std::is_same<exec_memory_space, Kokkos::HostSpace>::value) {
        ...
        if (_mb < 0) {
          if (max_dense_size < 256)
            _mb = -1;
          else if (max_dense_size < 512)
            _mb = 64;
          else if (max_dense_size < 1024)
            _mb = 128;
          else if (max_dense_size < 8192)
            _mb = 256;
          else
            _mb = 256;
        }
...
template <typename VT, typename ST> int Solver<VT, ST>::factorize_ldl(const value_type_array &ax) {
  if (_verbose) {
    printf("TachoSolver: Factorize\n");
    printf("======================\n");
  }
...
template <typename VT, typename ST>
int Solver<VT, ST>::solve(const value_type_matrix &x, const value_type_matrix &b, const value_type_matrix &t) {
  switch (_mode) {
  case Cholesky:
    solve_chol(x, b, t);
    break;
...
template <typename VT, typename ST>
int Solver<VT, ST>::solve_chol(const value_type_matrix &x, const value_type_matrix &b, const value_type_matrix &t) {
  if (_verbose) {
  ...
  if (_m < _small_problem_thres) {
    ...
    if (_levelset) {
      if (_variant == 0)
        _L0->solveCholesky(x, b, tt, _verbose);
      else if (_variant == 1)
        _L1->solveCholesky(x, b, tt, _verbose);
      else if (_variant == 2)
        _L2->solveCholesky(x, b, tt, _verbose);
    }
#if !defined(KOKKOS_ENABLE_CUDA)
    else if (nthreads == 1) {
...
template <typename VT, typename ST>
int Solver<VT, ST>::solve_ldl(const value_type_matrix &x, const value_type_matrix &b, const value_type_matrix &t) {
  if (_verbose) {
    printf("TachoSolver: Solve\n");
    printf("==================\n");
  }
...
template <typename VT, typename ST>
double Solver<VT, ST>::computeRelativeResidual(const value_type_array &ax, const value_type_matrix &x,
...
template <typename VT, typename ST> int Solver<VT, ST>::exportFactorsToCrsMatrix(crs_matrix_type &A) {
  if (_m < _small_problem_thres) {
...
template <typename VT, typename ST> int Solver<VT, ST>::release() {
  if (_verbose) {
